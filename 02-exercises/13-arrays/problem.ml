...
let double arr : unit = Array.iteri ~f:(fun i a -> arr.(i) <- a*2) arr
...
let double_selectively arr indices : unit =
  List.iter indices ~f:(fun i -> arr.(i) <- arr.(i) * 2)
...
let double_matrix matrix : unit = 
  let double_arr arr =
    Array.iteri arr ~f:(fun i a -> arr.(i) <- a * 2 ) in
  Array.iter matrix ~f:double_arr
